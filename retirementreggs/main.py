...
for gender in ('_male_3', '_male_2'):

	title = 'baseline_temp'+gender 

	with open('deterministic_params{}.csv'.format(gender), newline='') as pscfile:
	...
	lambdas  			= genfromtxt('lambdas_male.csv'.format(gender), delimiter=',')[1:]
	survival 			= genfromtxt('survival_probabilities_male.csv'.format(gender), delimiter=',')[0:]
...
#		for i in range(int(og.T-62)):#NO MAGIC NUMBERS HERE!
			...
			print("Solved value function for age %s in %s seconds "%(t, end_time-start_time ))
				...
				#pickle.dump(EXV_bar, open(".mod".format('EXV_bar{}').format(gender),"wb"))
				#pickle.dump(og, open(".mod".format('og'+gender),"wb"))
		#if t == 64:

	#og.TSALL 	= genprofiles(og, N=10000)
	#og.moments 	= gen_moments(og.TSALL)
	#moments_sorted = sortmoments(og.moments)
	#moments_sorted.to_csv("/scratch/pv33/moments_sorted{}csv".format(sys.argv[1]))

	#save results on Gadi scratch drive 
pickle.dump(og, open("/scratch/pv33/{}.mod".format(title),"wb"))
