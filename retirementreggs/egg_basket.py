...
from kambli import rand_grid_generator
...
#        for t in range(len(self.P_h)-1):
 #           self.P_h[t+1]       = (1+self.r_H)*self.P_h[t]/ (1-self.delta_housing)     
        ...
        # prepare random number grids for bellman grid-search

        # list of discrete\times continuous states 
        X_all_list              = cartesian([np.arange(len(self.X_disc)),np.arange(len(self.X_W_contgp))])


        print("Number of grid points is {}".format((int(self.T-self.tzero)*2*int(len(X_all_list)))))


        # random numbers for no adjustment 
        self.rand_grid_nadj = rand_grid_generator((int(self.T-self.tzero),2, int(len(X_all_list)), 1,10))

        # random numbers for adjustment 
        self.rand_grid_adj = rand_grid_generator((int(self.T-self.tzero),2, len(X_all_list),2,10))

        end = time.time()

        print("Initializsing model took {}, total grid points are {}".format(end-start, (int(self.T-self.tzero)*2*int(len(X_all_list)))))
...
def housingmodel_operator_factory(og, parallel_flag=True):
    ...
    def bellman_operator(t, EVX_bar, account_type):
        ...
        def solve_vfunc_R(state_ind):
            ...
            #@njit
            maximand = lambda x:   -objective_R(x[0],     # Current period consumption (C_{t}) choice
        ...
        def solve_vfunc_W(state_ind):
            ...
            wage                = y(t,X_disc_vals[state[0]][0])*int(t<R)
            DB_payout           = DB_benefit(t, t-tzero, y(t,X_disc_vals[state[0]][0]), X_disc[state[0]][0], P_E,P_stat,E)
            h0                  = x_cont_vals[-2]
            P_h                 = x_cont_vals[-1] 
            ...
            #@njit
            maximand = lambda x:   -objective_W(x[0],     # Current period consumption (C_{t}) choice
                                          x[1],         # Current period housing (H_{t}) choice 
                                          EXV_bar_disc, # Next period value function (V_{t+1})# do not confuse with voluntary contribution gr
                                          state[0],     # Current discrete state. Set to 0 if t>R. This an index of the discrete cartesian grid NOT (np.arange(len(X_disc))) the val. 
                                          state[1],     # Current continuous state. This is an index in the nodes grids generated from the continuous grid obect 
        ...
        if t>=R:
            ray_res         = np.array(p.map(solve_vfunc_R, np.arange(len(X_all_list))))
        ...
        if t<R:
            ray_res         = np.array(p.map(solve_vfunc_W, np.arange(len(X_all_list))))
